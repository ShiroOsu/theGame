...
public class Player extends Mob implements KeyListener {
	...
	/*
	 * updates players movement, (up, down, left and right)
	 */
	@Override
	public void update() {
		if(up){
		...
		if(down){
			y += speed;
			if(y + 80 > 700){ // if player tries to go out of the screen (y - direction)
				y = 620;
			}
		}
		if(left){
		...
		if(right){
			x += speed;
			if(x + 80 > 600){ // if player tries to go out of the screen (x - direction)
				x = 520;
			}
		}
	}
